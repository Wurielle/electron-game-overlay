...
- [x] Replace the demo's reactive WMI injection with a prearmed Steam-path watcher and pass normal-client Gun Frog launch/relaunch.
  - The former WMI `WITHIN 1` path loaded ReShade after Gun Frog had created its primary DXGI swap chain, so injection logs could report success while no transported scene appeared. At this milestone WMI supplied status/lifecycle evidence while a patched native injector snapshotted existing PIDs, polled for the next case-insensitive `\\steamapps\\` path match, excluded Unity crash-handler helpers, and reported the selected absolute path and PID before injecting. The later continuous observer milestone below superseded that one-shot arrangement; WMI is now fallback-only.
...
- [x] Replace WMI as the hot creation path with continuous native multi-process observation and an explicit ready handshake.
  - The parent-scoped injector observer remains armed while exact-PID attempts are in flight, reports matching creation/deletion identities through a strict UTF-8 protocol, and exits with its forked watcher parent. WMI is now created only as the slower fallback if the native observer fails.
...
- [ ] Add bounded age/count retention or startup scavenging for consumed SDK run directories.
  - Successful and failed exact-PID attempts intentionally retain logs as evidence today. The demo attempts every detected Steam executable, so long-running use can otherwise accumulate those consumed directories without a bound; unused prepared and pre-spawn-cancelled directories are already removed.
