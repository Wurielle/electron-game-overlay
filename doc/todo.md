...
- [x] Replace the demo's one-shot Steam-path selection with independent exact-PID attempts for every detected Steam executable and pass LORT's launcher/renderer chain.
...
- [ ] If deterministic pre-graphics coverage is required beyond the demo's inject-all WMI attempts, add continuous native multi-process observation with an explicit ready handshake.
  - `STEAM_GAME_AUTO_ATTACH_INJECTING` follows the WMI creation event and still precedes runtime staging and injector spawn. Independent exact-PID launchers close the selection/rearm gap, but a fast target can initialize graphics before its own injector runs.
  - A future native observer must keep watching while attempts are in flight, emit structured per-PID results, and preserve isolated staging/log ownership without reintroducing executable classification.
- [ ] Measure permitted WMI-notification and staging latency per supported target. Inject-all exact-PID attempts passed LORT's launcher/renderer chain, but an ordinary process observer still cannot provide the deterministic pre-graphics ordering of a client-owned `CREATE_SUSPENDED` launch.
