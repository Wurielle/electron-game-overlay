...
- [x] Replace the demo's reactive WMI injection with a prearmed Steam-path watcher and pass normal-client Gun Frog launch/relaunch.
  - The former WMI `WITHIN 1` path loaded ReShade after Gun Frog had created its primary DXGI swap chain, so injection logs could report success while no transported scene appeared. WMI now supplies demo status/lifecycle evidence only. A patched native injector snapshots existing PIDs, polls for the next case-insensitive `\\steamapps\\` path match, excludes Unity crash-handler helpers, and reports the selected absolute path and PID before injecting.
...
- [ ] Carry process creation identity end to end through exact-PID requests and WMI lifecycle correlation.
  - The native Steam-path watcher's ignore-baseline now retains handles for exact process objects, so sequential launch/relaunch cannot skip a new process solely because Windows reused a baseline PID. Exact-PID selection and WMI create/delete correlation still use PID plus executable basename and need an equivalent creation nonce before stale lifecycle events are considered fully hardened.
- [ ] Add an explicit SDK/native arm-ready handshake and close the one-shot rearm gap for overlapping or multi-process launches.
  - `STEAM_GAME_AUTO_ATTACH_ARMING` currently means the asynchronous attach was requested; it precedes session readiness, runtime staging, injector spawn, and the native flushed `ReShade path watcher armed.` marker. A target launched inside that narrow startup gap can be included in the native baseline and ignored.
  - The demo rearms after authenticated connection proof. A second matching Steam process launched while the first selection is injecting/authenticating can therefore already exist when the next watcher snapshots its baseline. Use a continuous native creation stream or rearm at selection time with explicit duplicate-injection protection before claiming concurrent/multi-process coverage.
- [ ] Measure permitted unsuspended watcher latency per supported target. Prearming removed WMI's roughly one-second delay and passed sequential Gun Frog launch/relaunch, but an ordinary process observer still cannot provide the deterministic pre-graphics ordering of a client-owned `CREATE_SUSPENDED` launch.
...
- [ ] Add safe texture retirement and production routing for multiple simultaneous target processes/swap chains. The SDK state model is keyed by PID/surface ID, but the production rendezvous remains single-target and process-level FPS still needs a primary-surface policy for multi-swap-chain games.
