...
test('the SDK resolves the ReShade runtime used by the demo', () => {
  ...
  assert.equal(path.basename(config.x86InjectorPath), 'inject32.exe');
  assert.equal(
    path.basename(config.addonManagerPath),
    'electron_game_overlay_reshade_manager.exe',
  );
  assert.equal(path.basename(config.runtimePath), 'ReShade64.dll');
  assert.equal(path.basename(config.x86RuntimePath), 'ReShade32.dll');
  assert.equal(
    path.basename(config.packageBuildStampPath),
    'electron_game_overlay_runtime.build.json',
  );
  assert.equal(
    path.basename(config.x86PackageBuildStampPath),
  ...
  assert.equal(
    path.basename(config.x86AddonPath),
