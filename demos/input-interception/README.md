...
```powershell
npm run demo:input -- --target-process="game.exe"
```

Wait for `Injector watcher ready. Launch game.exe now.` in the terminal, then
launch the game. The message confirms the native name watcher is armed before
process and graphics initialization. Once connected, press `Ctrl+I`; the badge
changes immediately to show the requested state. The shortcut is registered with
Electron's `globalShortcut`, which is the pattern a real desktop application
can use without hard-coding a key in the SDK.
