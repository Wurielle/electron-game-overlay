...
use crate::{hook_ejection_guard, perform_eject, util, Hooks, ImguiRenderLoop, EJECT_REQUESTED};
...
unsafe extern "system" fn dx9_present_impl(
    ...
    let _hook_ejection_guard = hook_ejection_guard();
...
unsafe extern "system" fn dx9_reset_impl(
    ...
    if let Some(pipeline) = PIPELINE.get() {
        if let Err(error) = pipeline.lock().cleanup() {
            error!(?error, "Could not detach the DirectX 9 pipeline before reset");
            return dx9_reset(this, present_params);
        }
    }

    if let Some(pipeline) = PIPELINE.take() {
        let render_loop = pipeline.into_inner().into_render_loop();
...
impl Hooks for ImguiDx9Hooks {
    ...
    unsafe fn unhook(&mut self) -> Result<()> {
        if let Some(pipeline) = PIPELINE.get() {
            pipeline.lock().cleanup()?;
        }

        TRAMPOLINES.take();
        if let Some(pipeline) = PIPELINE.take() {
            drop(pipeline.into_inner().into_render_loop());
        }
        RENDER_LOOP.take();
        Ok(())
    }
