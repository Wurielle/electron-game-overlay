...
impl MhHook {
    ...
    /// Remove this disabled hook from MinHook's hook table.
    ///
    /// # Safety
    ///
    /// No callback may still be using the hook or its trampoline.
    pub unsafe fn remove(&self) -> Result<(), MH_STATUS> {
        MH_RemoveHook(self.addr).ok_context("MH_RemoveHook")
    }
}
