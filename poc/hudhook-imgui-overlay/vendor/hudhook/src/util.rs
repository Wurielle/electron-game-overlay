...
impl HookEjectionBarrier {
    ...
    pub fn acquire_ejection_guard(&self) -> RwLockReadGuard<'_, ()> {
        // A guarded graphics callback can call another hooked API. Recursive
        // reads must not queue behind teardown while the outer read is still
        // held, or neither side could make progress.
