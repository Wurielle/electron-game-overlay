...
use crate::sync_input::{RawInputData, RawKeyboardInput, RawMouseInput};
...
fn handle_raw_mouse_input(io: &mut Io, raw_mouse: &RawMouseInput) {
    let button_flags = raw_mouse.button_flags as u32;
    ...
    let wheel_delta_y = if button_flags & RI_MOUSE_WHEEL != 0 {
        let wheel_delta = raw_mouse.button_data as i16 / WHEEL_DELTA as i16;
        wheel_delta as f32
    } else {
        0.0
    };
    ...
    let wheel_delta_x = if button_flags & RI_MOUSE_HWHEEL != 0 {
        let wheel_delta = raw_mouse.button_data as i16 / WHEEL_DELTA as i16;
        wheel_delta as f32
    } else {
        0.0
    ...
    if raw_mouse.cursor_valid {
        io.add_mouse_pos_event([raw_mouse.cursor_x as f32, raw_mouse.cursor_y as f32]);
        return;
    }

    let mouse_flags = raw_mouse.flags;
    let (last_x, last_y) = (raw_mouse.last_x as f32, raw_mouse.last_y as f32);

    if (mouse_flags & MOUSE_MOVE_ABSOLUTE.0) != 0 {
...
fn handle_raw_keyboard_input(io: &mut Io, raw_keyboard: &RawKeyboardInput) {
    // Ignore messages without a valid key code
    if raw_keyboard.virtual_key == 0 {
        return;
    }
    ...
    let scan_code = {
        let mut code = raw_keyboard.make_code as u32;
    ...
    let is_key_down = flags & RI_KEY_BREAK == 0;
    let is_key_up = flags & RI_KEY_BREAK != 0;

    // Map the virtual key if necessary.
    let virtual_key = match VIRTUAL_KEY(raw_keyboard.virtual_key) {
...
fn handle_raw_input(io: &mut Io, WPARAM(wparam): WPARAM, raw_input: Option<RawInputData>) {
    ...
    match raw_input {
        Some(RawInputData::Mouse(mouse)) => handle_raw_mouse_input(io, &mouse),
        Some(RawInputData::Keyboard(keyboard)) => handle_raw_keyboard_input(io, &keyboard),
        _ => {}
    }
}
...
pub fn imgui_wnd_proc_impl<T: RenderEngine>(
    ...
    raw_input: Option<RawInputData>,
    pipeline: &mut Pipeline<T>,
) {
    if pipeline
        .render_loop()
        .before_wnd_proc(hwnd, umsg, WPARAM(wparam), LPARAM(lparam))
        == BeforeWndProc::Break
    {
        pipeline
            .render_loop()
            .after_wnd_proc(hwnd, umsg, WPARAM(wparam), LPARAM(lparam));
        return;
    }
    ...
    match umsg {
        WM_INPUT => handle_raw_input(io, WPARAM(wparam), raw_input),
