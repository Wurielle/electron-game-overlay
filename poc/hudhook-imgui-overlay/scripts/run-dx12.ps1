...
$HostExecutable = Join-Path $RunDirectory "d3d12_overlay_test_host.exe"
$Injector = Join-Path $RunDirectory "hudhook_overlay_injector.exe"
$Payload = Join-Path $RunDirectory "hudhook_imgui_overlay_dx12.dll"
...
foreach ($Artifact in @($HostExecutable, $Injector, $Payload)) {
    if (-not (Test-Path $Artifact -PathType Leaf)) {
        throw "POC artifact not found: $Artifact. Run build-dx12.ps1 first."
    }
}
...
$ExistingHosts = Get-Process -Name "d3d12_overlay_test_host" -ErrorAction SilentlyContinue
if ($ExistingHosts) {
...
try {
    ...
    $LogPath = Join-Path $RunDirectory "hudhook_imgui_overlay_dx12-$($HostProcess.Id).log"
    ...
    Write-Host "Press Escape in the host to close it."
