...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Configuring the controlled D3D12 host failed with exit code $LASTEXITCODE."
    }

    & $CMake --build --preset relwithdebinfo-dx12
    if ($LASTEXITCODE -ne 0) {
        throw "Building the controlled D3D12 host failed with exit code $LASTEXITCODE."
    }
}
finally {
    Pop-Location
...
$HostSource = Join-Path $RepoRoot "build\electron-game-overlay-runtime\RelWithDebInfo\d3d12_overlay_test_host.exe"
$CargoRelease = Join-Path $CargoTarget "x86_64-pc-windows-msvc\release"
$PayloadSource = Join-Path $CargoRelease "hudhook_imgui_overlay_dx12.dll"
...
$RunDirectory = Join-Path $RepoRoot "build\hudhook-imgui-overlay\run\dx12"
...
Copy-Item -Force $HostSource (Join-Path $RunDirectory "d3d12_overlay_test_host.exe")
Copy-Item -Force $PayloadSource (Join-Path $RunDirectory "hudhook_imgui_overlay_dx12.dll")
...
Write-Host ""
Write-Host "hudhook D3D12 POC staged in:"
Write-Host "  $RunDirectory"
Write-Host ""
Write-Host "Run it with:"
Write-Host "  .\poc\hudhook-imgui-overlay\scripts\run-dx12.ps1"
