    ...
    match catch_unwind(AssertUnwindSafe(operation)) {
        ...
        Err(payload) => {
            set_last_error(format!(
                "electron-overlay-core panicked across an ABI boundary: {}",
...
unsafe fn core_ref<'a>(core: *const EgoCore) -> FfiResult<&'a EgoCore> {
    core.as_ref()
        .ok_or_else(|| FfiError::new(EGO_STATUS_INVALID_ARGUMENT, "core pointer is null"))
}
...
/// # Safety
/// `out_core` must be writable. On success, its returned handle must eventually
/// be passed exactly once to [`ego_core_destroy`].
#[no_mangle]
pub unsafe extern "C" fn ego_core_create(abi_version: u32, out_core: *mut *mut EgoCore) -> i32 {
    ffi_call(|| {
        let out_core = out_core.as_mut().ok_or_else(|| {
            FfiError::new(EGO_STATUS_INVALID_ARGUMENT, "out_core pointer is null")
        })?;
        *out_core = ptr::null_mut();
        ...
        let bridge = ElectronFrameBridge::spawn().map_err(|error| {
            FfiError::new(
                ...
                format!("cannot initialize Electron overlay core: {error}"),
            )
        })?;
        *out_core = Box::into_raw(Box::new(EgoCore { bridge }));
        Ok(())
    })
}

/// # Safety
/// `core` must be a live handle returned by [`ego_core_create`]. No other ABI
/// call may use it concurrently, and it must not be destroyed more than once.
#[no_mangle]
pub unsafe extern "C" fn ego_core_destroy(core: *mut EgoCore) -> i32 {
    ffi_call(|| {
        if core.is_null() {
            return Err(FfiError::new(
                EGO_STATUS_INVALID_ARGUMENT,
...
/// # Safety
/// `core` must remain live for the call and `out_snapshot` must be writable.
/// The returned snapshot has independent lifetime and must be released once.
#[no_mangle]
pub unsafe extern "C" fn ego_core_acquire_scene(
    core: *const EgoCore,
    ...
    ffi_call(|| {
        ...
        *out_snapshot = Box::into_raw(Box::new(make_scene_snapshot(core.bridge.scene())?));
        Ok(())
    })
}
...
/// [`ego_core_acquire_scene`] and must not be released more than once.
#[no_mangle]
...
/// # Safety
/// `core` must remain live for the call and `out_desired` must be writable.
#[no_mangle]
pub unsafe extern "C" fn ego_core_desired_interception(
    core: *const EgoCore,
    out_desired: *mut u32,
) -> i32 {
    ffi_call(|| {
        ...
        *out_desired = bool_u32(core.bridge.desired_interception());
        Ok(())
    })
}

/// # Safety
/// `core` must remain live and must not be destroyed concurrently.
#[no_mangle]
pub unsafe extern "C" fn ego_core_set_target_focused(core: *mut EgoCore, focused: u32) -> i32 {
    ffi_call(|| {
        let core = core_ref(core)?;
        core.bridge.set_target_focused(focused != 0);
        Ok(())
    })
}
...
/// `core` must remain live and must not be destroyed concurrently.
#[no_mangle]
pub unsafe extern "C" fn ego_core_apply_input_filter(
    core: *mut EgoCore,
    routing_enabled: u32,
    acknowledge: u32,
) -> i32 {
...
/// # Safety
/// `core` must remain live for the call. `inout_state` must point to a writable
/// record initialized with its size and ABI version.
#[no_mangle]
pub unsafe extern "C" fn ego_core_get_input_state(
    core: *const EgoCore,
    ...
    ffi_call(|| {
        ...
        let state = core.bridge.input_state();
        ...
        *inout_state = EgoInputStateV1 {
            ...
            desired_interception: bool_u32(core.bridge.desired_interception()),
...
/// # Safety
/// `core` must remain live for the call. `target_hwnd` must identify a live
/// target window for any message whose coordinate conversion uses that HWND.
#[no_mangle]
pub unsafe extern "C" fn ego_core_route_window_message(
    core: *mut EgoCore,
    target_hwnd: usize,
    message: u32,
    wparam: u64,
    ...
    ffi_call(|| {
        ...
        core.bridge.route_window_message(
...
mod tests {
    ...
    fn null_arguments_return_status_and_error_text() {
        let status = unsafe { ego_core_destroy(ptr::null_mut()) };
