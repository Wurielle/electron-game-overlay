#ifndef ELECTRON_OVERLAY_CORE_H
#define ELECTRON_OVERLAY_CORE_H

#include <stddef.h>
#include <stdint.h>

#if !defined(_WIN64)
#error "electron-overlay-core supports x64 Windows only"
#endif
...
typedef struct ego_core ego_core;
...
ego_status EGO_CALL ego_core_create(uint32_t abi_version, ego_core **out_core);
ego_status EGO_CALL ego_core_destroy(ego_core *core);

ego_status EGO_CALL ego_core_acquire_scene(
    const ego_core *core,
...
ego_status EGO_CALL ego_core_desired_interception(
    const ego_core *core,
    uint32_t *out_desired);
ego_status EGO_CALL ego_core_set_target_focused(ego_core *core, uint32_t focused);
ego_status EGO_CALL ego_core_apply_input_filter(
    ego_core *core,
    uint32_t routing_enabled,
    uint32_t acknowledge);
ego_status EGO_CALL ego_core_get_input_state(
    const ego_core *core,
    ego_input_state_v1 *inout_state);

/* Observes/routes a copied message. The status is not an input-block decision. */
ego_status EGO_CALL ego_core_route_window_message(
    ego_core *core,
